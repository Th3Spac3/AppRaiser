...
module WorkImage
  ...
  def show_image(theme_id, image_index)
    ...
    if user_valued == 1
      common_average_value = Image.find_image(image_id).average_value
      if common_average_value.blank?
        common_average_value = 0
      end
       common_average_value.round unless common_average_value.blank?
     logger.info "In 2show_image: common_average_value = #{common_average_value.inspect} "
    else
      common_average_value = 0
    end
    data = { index: image_index,
             ...
             value: value,
             common_average_value: common_average_value
    }
